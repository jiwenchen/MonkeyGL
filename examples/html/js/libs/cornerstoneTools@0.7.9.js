        ...
        switch (e.type) {
            ...
            case "panmove":
                ...
                pageDistanceMoved += Math.sqrt(
                    deltaPoints.page.x * deltaPoints.page.x + deltaPoints.page.y * deltaPoints.page.y
                );
            ...
            handleMover(
                ...
                function() {
                    ...
                    $(element).on(
                        ...
                        eventData,
                        mouseToolInterface.mouseMoveCallback || mouseMoveCallback
                    );
                    $(element).on(
                        "CornerstoneToolsMouseDown",
                        eventData,
                        mouseToolInterface.mouseDownCallback || mouseDownCallback
                    );
                    $(element).on(
                        "CornerstoneToolsMouseDownActivate",
                        eventData,
                    ...
                    if (mouseToolInterface.mouseDoubleClickCallback) {
                        $(element).on(
                            ...
                            eventData,
                            mouseToolInterface.mouseDoubleClickCallback
                        );
                    }
        ...
        function mouseDownCallback(e, eventData) {
            ...
            if (cornerstoneTools.isMouseButtonEnabled(eventData.which, e.data.mouseButtonMask)) {
                ...
                if (toolData !== undefined) {
                    for (i = 0; i < toolData.data.length; i++) {
                        data = toolData.data[i];
                        var handle = cornerstoneTools.getHandleNearImagePoint(
                            eventData.element,
                            data.handles,
                            coords,
                            distanceSq
...
(function($, cornerstone, cornerstoneMath, cornerstoneTools) {
    ...
    function touchTool(touchToolInterface) {
        ...
        function addNewMeasurement(touchEventData) {
            ...
            $(element).off(
                "CornerstoneToolsTouchStartActive",
                touchToolInterface.touchDownActivateCallback || touchDownActivateCallback
            );
            ...
            cornerstoneTools.moveNewHandleTouch(
                ...
                function() {
                    ...
                    $(element).on(
                        ...
                        touchToolInterface.touchStartCallback || touchStartCallback
                    );
                    cornerstone.updateImage(element);
                }
            );
            ...
            if (toolData) {
                for (i = 0; i < toolData.data.length; i++) {
                    ...
                    if (handle) {
                        ...
                        handle.active = true;
                        cornerstone.updateImage(element);
                        cornerstoneTools.touchMoveHandle(
                            e,
                            touchToolInterface.toolType,
                            data,
                            handle,
            ...
            if (toolData && touchToolInterface.pointNearTool) {
                for (i = 0; i < toolData.data.length; i++) {
                    ...
                    if (touchToolInterface.pointNearTool(element, data, coords)) {
                        ...
                        data.active = true;
                        cornerstone.updateImage(element);
                        cornerstoneTools.touchMoveAllHandles(
                            e,
                            data,
                            toolData,
                            touchToolInterface.toolType,
    ...
    function onImageRendered(e, eventData) {
        ...
        for (var i = 0; i < toolData.data.length; i++) {
            ...
            if (config && config.shadow) {
                ...
                context.shadowBlur = config.shadowBlur || 2;
            }
            ...
            var dx1 =
                (Math.ceil(data.handles.start.x) - Math.ceil(data.handles.end.x)) * eventData.image.columnPixelSpacing;
            var dy1 =
                (Math.ceil(data.handles.start.y) - Math.ceil(data.handles.end.y)) * eventData.image.rowPixelSpacing;
            var dx2 =
                (Math.ceil(data.handles.start2.x) - Math.ceil(data.handles.end2.x)) *
                eventData.image.columnPixelSpacing;
            var dy2 =
                (Math.ceil(data.handles.start2.y) - Math.ceil(data.handles.end2.y)) * eventData.image.rowPixelSpacing;

            var angle = Math.acos(
                Math.abs(
                    (dx1 * dx2 + dy1 * dy2) / (Math.sqrt(dx1 * dx1 + dy1 * dy1) * Math.sqrt(dx2 * dx2 + dy2 * dy2))
                )
            );
...
(function($, cornerstone, cornerstoneMath, cornerstoneTools) {
    ...
    function addNewMeasurement(mouseEventData) {
        ...
        function doneChangingTextCallback(text) {
            ...
            $(mouseEventData.element).on(
                "CornerstoneToolsMouseMove",
                eventData,
                cornerstoneTools.arrowAnnotate.mouseMoveCallback
            );
            $(mouseEventData.element).on(
                "CornerstoneToolsMouseDown",
                eventData,
                cornerstoneTools.arrowAnnotate.mouseDownCallback
            );
            $(mouseEventData.element).on(
                "CornerstoneToolsMouseDownActivate",
                eventData,
                cornerstoneTools.arrowAnnotate.mouseDownActivateCallback
            );
            $(mouseEventData.element).on(
                "CornerstoneToolsMouseDoubleClick",
                eventData,
                cornerstoneTools.arrowAnnotate.mouseDoubleClickCallback
            );
        }
        ...
        cornerstone.updateImage(mouseEventData.element);
        cornerstoneTools.moveNewHandle(
            mouseEventData,
            toolType,
            measurementData,
            measurementData.handles.end,
    ...
    function addNewMeasurementTouch(touchEventData) {
        ...
        function doneChangingTextCallback(text) {
            ...
            $(element).on(
                "CornerstoneToolsTouchStartActive",
                cornerstoneTools.arrowAnnotateTouch.touchDownActivateCallback
            );
    ...
    function doubleClickCallback(e, eventData) {
        ...
        if (
            e.data &&
            e.data.mouseButtonMask &&
            !cornerstoneTools.isMouseButtonEnabled(eventData.which, e.data.mouseButtonMask)
        ) {
            return false;
        }
...
(function($, cornerstone, cornerstoneTools) {
    ...
    function minimalStrategy(eventData) {
        ...
        if (eventData.isTouchEvent === true) {
            toolCoords = cornerstone.pageToPixel(
                element,
...
(function($, cornerstone, cornerstoneMath, cornerstoneTools) {
    ...
    function pointInEllipse(ellipse, location) {
        ...
        var inEllipse =
            (normalized.x * normalized.x) / (xRadius * xRadius) + (normalized.y * normalized.y) / (yRadius * yRadius) <=
            1.0;
        return inEllipse;
    }

    function calculateMeanStdDev(sp, ellipse, isPerfusion) {
        ...
        for (var y = ellipse.top; y < ellipse.top + ellipse.height; y++) {
            for (var x = ellipse.left; x < ellipse.left + ellipse.width; x++) {
                if (
                    ...
                    let spVal = isPerfusion ? sp[index] / 100 : sp[index];
                    sum += spVal;
                    sumSquared += spVal * spVal;
                    count++;
                }
    ...
    function onImageRendered(e, eventData) {
        ...
        var modalityTag = "x00080060";
        let isPerfusion = eventData.image.imageId.indexOf(":perfusion_") > -1;
        var modality;
        if (image.data) {
        ...
        for (var i = 0; i < toolData.data.length; i++) {
            ...
            var area, meanStdDev, meanStdDevSUV;
            ...
            if (!image.color) {
                ...
                meanStdDev = calculateMeanStdDev(pixels, ellipse, isPerfusion);
                ...
                if (modality === "PT") {
                    ...
                    meanStdDevSUV = {
                        ...
                        stdDev: cornerstoneTools.calculateSUV(
                            image,
...
(function($, cornerstone, cornerstoneMath, cornerstoneTools) {
    ...
    function endDrawing(eventData, handleNearby) {
        ...
        config.endDrawingCallBack && config.endDrawingCallBack(toolData.data);
    }
        ...
        for (var i = 0; i < toolData.data.length; i++) {
            ...
            if (data.handles.length) {
                for (var j = 0; j < data.handles.length; j++) {
                    ...
                    var mouseLocationCanvas = cornerstone.pixelToCanvas(
                        eventData.element,
...
(function($, cornerstone, cornerstoneTools) {
    ...
    function drawMagnificationTool(eventData) {
        ...
        zoomCtx.drawImage(
            ...
            copyFrom.x,
            copyFrom.y,
            canvas.width - copyFrom.x,
            canvas.height - copyFrom.y,
            0,
            0,
            scaledMagnify.x,
            scaledMagnify.y
...
// Begin Source: src/imageTools/page.js
(function($, cornerstone, cornerstoneTools) {
    "use strict";
    ...
    cornerstoneTools.page = cornerstoneTools.simpleMouseButtonTool(mouseDownCallback);
    cornerstoneTools.pageTouchDrag = cornerstoneTools.touchDragTool(dragCallback);
})($, cornerstone, cornerstoneTools);

// End Source; src/imageTools/page.js
...
(function($, cornerstone, cornerstoneTools) {
    ...
    function onImageRendered(e, eventData) {
        ...
        for (var i = 0; i < toolData.data.length; i++) {
            ...
            // draw the handles
            // var cpoint = cornerstone.pixelToCanvas(eventData.element, data.handles.end);
            // context.beginPath();
            // context.strokeStyle = color;
            // context.lineWidth = 2;
            // context.moveTo(cpoint.x - 8, cpoint.y);
            // context.lineTo(cpoint.x + 8, cpoint.y);
            // context.moveTo(cpoint.x, cpoint.y - 8);
            // context.lineTo(cpoint.x, cpoint.y + 8);
            // context.stroke();
            cornerstoneTools.drawHandles(context, eventData, data.handles, color, null, 6);
            ...
            if (eventData.image.color) {
                ...
                text = "X:" + x + ", " + "Y:" + y;
                if (!isPerfusion) {
                    str = " Value: " + parseFloat(mo.toFixed(3));
                } else {
                    str = "Value: " + ~~sp / 100;
                }
                // str = "MO: " + parseFloat(mo.toFixed(3));
                if (suv) {
                    str += " SUV: " + parseFloat(suv.toFixed(3));
                }
            }
            ...
            if (data.handles.textBox.hasMoved) {
                ...
                link.start = cornerstone.pixelToCanvas(eventData.element, data.handles.end);
...
(function($, cornerstone, cornerstoneMath, cornerstoneTools) {
    ...
    function onImageRendered(e, eventData) {
        ...
        for (var i = 0; i < toolData.data.length; i++) {
            ...
            var meanStdDev = calculateMeanStdDev(pixels, ellipse, isPerfusion);
            ...
            // Calculate the image area from the ellipse dimensions and pixel spacing
            var area = width * columnPixelSpacing * (height * rowPixelSpacing);
            // var area = (width * eventData.image.columnPixelSpacing) * (height * eventData.image.rowPixelSpacing);

            var meanText = "Mean: " + meanStdDev.mean.toFixed(2) + moSuffix;
            var stdDevText = "StdDec: " + meanStdDev.stdDev.toFixed(2) + moSuffix;
            textLines.push(meanText);
            textLines.push(stdDevText);
            ...
            var areaText = "Area: " + area.toFixed(2) + suffix;
            textLines.push(areaText);
            ...
            function() {
                ...
                if (cornerstoneTools.anyHandlesOutsideImage(mouseEventData, measurementData.handles)) {
                    ...
                    $(element).on(
                        "CornerstoneToolsMouseDown",
                        eventData,
                        cornerstoneTools.simpleAngle.mouseDownCallback
                    );
                    $(element).on(
                        "CornerstoneToolsMouseDownActivate",
                        eventData,
                        cornerstoneTools.simpleAngle.mouseDownActivateCallback
                    );
                    cornerstone.updateImage(element);
                    return;
                }
            ...
            function() {
                ...
                cornerstoneTools.moveNewHandleTouch(
                    ...
                    function() {
                        ...
                        $(element).on(
                            "CornerstoneToolsTouchStart",
                            cornerstoneTools.simpleAngleTouch.touchStartCallback
                        );
                        $(element).on(
                            "CornerstoneToolsTouchStartActive",
                            cornerstoneTools.simpleAngleTouch.touchDownActivateCallback
...
(function($, cornerstone, cornerstoneTools) {
    ...
    function doubleClickCallback(e, eventData) {
        ...
        function doneChangingTextCallback(updatedText, deleteTool) {
            ...
            $(element).on(
                "CornerstoneToolsMouseDownActivate",
                mouseButtonData,
                cornerstoneTools.textMarker.mouseDownActivateCallback
            );
            $(element).on(
                "CornerstoneToolsMouseDoubleClick",
                mouseButtonData,
                cornerstoneTools.textMarker.mouseDoubleClickCallback
            );
        }
    ...
    function touchPressCallback(e, eventData) {
        ...
        function doneChangingTextCallback(updatedText, deleteTool) {
            ...
            $(element).on(
                "CornerstoneToolsTouchStartActive",
                cornerstoneTools.textMarkerTouch.touchDownActivateCallback
            );
...
(function($, cornerstone, cornerstoneMath, cornerstoneTools) {
    ...
    function applyWWWCRegion(eventData) {
        ...
        var minMaxMean = calculateMinMaxMean(
            pixelLuminanceData,
            eventData.image.minPixelValue,
            eventData.image.maxPixelValue
        );
...
(function($, cornerstone, cornerstoneTools) {
    ...
    function defaultStrategy(eventData, ticks) {
        ...
        // When it is 'vs' image, the maximum viewport scale as 6
        if (eventData.image.imageId.indexOf("plane_type_3d") > -1) {
            if (eventData.viewport.scale > 6) {
                eventData.viewport.scale = 6.0;
            }
        }
        cornerstone.setViewport(element, viewport);
        if (eventData.image.zoomCoord !== undefined) {
            return;
        }
    ...
    function translateStrategy(eventData, ticks) {
        ...
        if (ticks < 0) {
            ...
            if (Math.abs(viewport.translation.x) < minTranslation) {
                ...
                Math.abs(viewport.translation.x) < minTranslation &&
            ...
            if (Math.abs(distanceToDesired.x) < minTranslation) {
                ...
                Math.abs(distanceToDesired.x) < minTranslation &&
...
(function(cornerstone, cornerstoneTools) {
    ...
    function drawHandles(context, renderData, handles, color, options, radius) {
        var handleRadius = 1;
        if (radius) {
            handleRadius = radius;
        }
...
(function($, cornerstone, cornerstoneTools) {
    ...
    function moveNewHandleTouch(eventData, toolType, data, handle, doneMovingCallback, preventHandleOutsideImage) {
        ...
        var element = eventData.element;
        var imageCoords = cornerstone.pageToPixel(
            element,
            eventData.currentPoints.page.x,
...
(function($, cornerstone, cornerstoneTools) {
    ...
    function renderActiveReferenceLine(context, eventData, targetElement, referenceElement) {
        ...
        var referenceLine = cornerstoneTools.referenceLines.calculateReferenceLine(
            targetImagePlane,
            referenceImagePlane
        );
        if (!referenceLine) {
...
(function($, cornerstone, cornerstoneTools) {
    ...
    var resetPrefetchTimeout, resetPrefetchDelay;
...
(function($, cornerstone, cornerstoneTools) {
    ...
    function restoreApplicationState(appState) {
        // Make sure t
        if (
            !appState.hasOwnProperty("imageIdToolState") ||
...
(function(cornerstoneTools) {
    ...
    function loadHandlerManager() {
        var defaultStartLoadHandler, defaultEndLoadHandler, defaultErrorLoadingHandler;
...
(function(cornerstone, cornerstoneTools) {
    ...
    function addTimeSeriesStateManager(element, tools) {
        ...
        var timeSeriesSpecificStateManager = cornerstoneTools.newTimeSeriesSpecificToolStateManager(
            tools,
            oldStateManager
        );
        ...
        this.getDistances = function() {
            ...
            sourceElements.forEach(function(sourceElement) {
                ...
                targetElements.forEach(function(targetElement) {
                    ...
                    initialData.distances[sourceImageId][targetImageId] = targetImagePosition
                        .clone()
                        .sub(sourceImagePosition);
                });
...
(function($, cornerstone, cornerstoneTools) {
    ...
    function updateLineSample(measurementData) {
        ...
        measurementData.timeSeries.stacks.forEach(function(stack) {
            ...
            loader.then(function(image) {
                var offset =
                    Math.round(measurementData.handles.end.x) + Math.round(measurementData.handles.end.y) * image.width;
    ...
    function createNewMeasurement(mouseEventData) {
        ...
        if (
            timeSeriestoolData === undefined ||
            timeSeriestoolData.data === undefined ||
            timeSeriestoolData.data.length === 0
        ) {
            return;
        }
    ...
    function onImageRendered(e, eventData) {
        ...
        for (var i = 0; i < toolData.data.length; i++) {
            ...
            // Draw text
            var fontParameters = cornerstoneTools.setContextToDisplayFontSize(
...
(function(cornerstoneTools) {
    ...
    function calculateSUV(image, storedPixelValue) {
        ...
        var acquisitionTimeInSeconds =
            fracToDec(seriesAcquisitionTime.fractionalSeconds) +
            seriesAcquisitionTime.seconds +
            seriesAcquisitionTime.minutes * 60 +
            seriesAcquisitionTime.hours * 60 * 60;
        var injectionStartTimeInSeconds =
